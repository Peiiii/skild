# 文档

[English](./README.md) | 简体中文

---

## 快速开始

| 文档 | 说明 |
|------|------|
| **[快速上手](./getting-started.zh-CN.md)** | 2 分钟安装第一个 Skill |

---

## 用户指南

安装、管理和发现 AI Agent Skills。

| 文档 | 说明 |
|------|------|
| **[使用文档](./usage.zh-CN.md)** | CLI 端到端使用与工作流 |
| **[安装 Skills](./installing-skills.zh-CN.md)** | 从 GitHub、本地或 Registry 安装 |
| **[跨平台同步](./installing-skills.zh-CN.md#%E8%B7%A8%E5%B9%B3%E5%8F%B0%E5%90%8C%E6%AD%A5)** | 自动发现缺失并跨平台同步 |
| **[Skillsets 指南](./skillsets.zh-CN.md)** | 一键安装多个 Skills |
| **[Skild Hub 指南](./hub.zh-CN.md)** | Web 界面使用 |
| **[常见问题](./faq.md)** | 常见问题解答 |

---

## 作者指南

创建和发布你自己的 Skills。

| 文档 | 说明 |
|------|------|
| **[创建 Skills](./creating-skills.md)** | 构建你自己的 Skills |
| **[发布 Skills](./publishing.zh-CN.md)** | 分享给社区 |

---

## 从 GitHub 提交

发现了 GitHub 上的优质 Skill？帮助社区收录它。

| 文档 | 说明 |
|------|------|
| **[从 GitHub 提交](./submit-from-github.zh-CN.md)** | 收录 GitHub 上的 Skill |

---

## 维护者指南

| 文档 | 说明 |
|------|------|
| [部署指南](./deployment-guide.md) | 自托管 Registry |
| [发布流程](./release.md) | 如何发布新版本 |
| [设计文档](./design/) | 架构和设计决策 |
| [开发日志](./logs/) | 变更记录和进度 |

---

## 快速链接

- **官网**: [skild.sh](https://skild.sh)
- **Skild Hub**: [hub.skild.sh](https://hub.skild.sh)
- **GitHub**: [github.com/Peiiii/skild](https://github.com/Peiiii/skild)
- **npm**: [npmjs.com/package/skild](https://www.npmjs.com/package/skild)
