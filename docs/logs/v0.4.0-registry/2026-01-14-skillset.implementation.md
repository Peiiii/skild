...
后续补丁：
- `skild@0.4.1` / `@skild/core@0.4.1`：修复 GitHub URL 作为多 skill 源时的 `suggestedSource` 推导，确保 `--recursive` 与 `--json` 输出可直接复用安装
- `skild@0.4.2` / `@skild/core@0.4.2`：修复交互式安装提示被 spinner 覆盖的问题（多 skill 源会正确弹出确认提示）

---

## 补充迭代：精选 Skillsets（基于 awesome-claude-skills）

### 目标

- 把“网上已经火的 skills 集合”转化为可一键安装的 Skillsets
- 每个 Skillset 配一个短 alias，用户直接 `skild install <alias>`

### 已发布的精选 Skillsets（alias → registry）

- `superpowers` → `@peiiii/obra-superpowers-pack`
- `anthropics-skills` → `@peiiii/anthropics-official-pack`
- `claude-office` → `@peiiii/claude-office-pack`
- `claude-design` → `@peiiii/claude-design-pack`
- `claude-dev` → `@peiiii/claude-dev-pack`
- `claude-content` → `@peiiii/claude-content-pack`

### 验证与上线闭环

本地安装 smoke（隔离环境，避免污染真实 `~/.skild`）：

```
SKILD_HOME="$(mktemp -d)" pnpm -s cli install ./skillsets/claude-dev-pack --json
```

线上发布（registry）：

```
pnpm -s cli publish --dir ./skillsets/claude-dev-pack --alias claude-dev --json
```

线上冒烟（alias resolve + 真实安装）：

```
curl -fsS 'https://registry.skild.sh/resolve?alias=claude-dev'
SKILD_HOME="$(mktemp -d)" npx -y skild@0.4.2 install claude-dev --json
```
