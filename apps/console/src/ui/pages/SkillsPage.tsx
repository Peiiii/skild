  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <div className="text-2xl font-semibold">Discover Skills</div>
        <div className="text-sm text-muted-foreground">Search the registry and copy install commands.</div>
      </div>

      <form className="flex gap-2" onSubmit={onSubmit}>
        <Input value={query} onChange={e => setQuery(e.currentTarget.value)} placeholder="Search skills…" />
      ...
      <div className="grid gap-3">
            ...
            <Card key={item.name}>
              <CardHeader className="py-4">
                <CardTitle className="text-base">
                  {href ? <Link className="hover:underline" to={href}>{item.name}</Link> : item.name}
                </CardTitle>
              </CardHeader>
        ...
        {!busy && !error && items.length === 0 && <div className="text-sm text-muted-foreground">No results.</div>}
      </div>
    </div>
  );
}
